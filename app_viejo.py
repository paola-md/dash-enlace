...
button = html.Div(
    [
        dbc.Button("Obtener resultados", id="button_envia", outline=True, className="mr-3",color="primary"),
        html.Span(id="button_2", style={"vertical-align": "middle"}),
    ]
)

button_descarga = html.Div(
    [
...
layout_results =  html.Div(
    [
        ...
        dbc.Row(
            dbc.Col(button, align="center",width={"size":4, "offset":5}),
        ),
        ...
        dbc.Row(
            dbc.Col(button_descarga, align="center",width={"size":4, "offset":2}),
        ),
    ]
)
...
def update_results(n_clicks, value_tipo, value_estados, value_estado):
    #return n_clicks
    if n_clicks is None:
        raise PreventUpdate
    else: 
        ...
        return criteria, open(nombre_map, 'r').read(), update_link
